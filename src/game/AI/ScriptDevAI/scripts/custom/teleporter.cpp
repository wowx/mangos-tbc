...
bool GossipHello_Teleporter(Player *player, Creature *_Creature)
{
    ...
    if (player->GetTeam() == ALLIANCE)
        {
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, "|TInterface/ICONS/Spell_arcane_teleportstormwind:30:30:-18:0|t Alliance Shop Zone (Stormwind)", GOSSIP_SENDER_MAIN, 1001);
        }
    if (player->GetTeam() == HORDE)
        {
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, "|TInterface/ICONS/Spell_arcane_teleportorgrimmar:30:30:-18:0|t Horde Shop Zone (Orgrimmar)", GOSSIP_SENDER_MAIN, 1006);
        }
    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, "|TInterface/ICONS/Inv_misc_shadowegg:30:30:-18:0|t Start Location (Dark Portal)", GOSSIP_SENDER_MAIN, 5003);
    ...
    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, "|TInterface/ICONS/Ability_creature_cursed_04:30:30:-18:0|t Other", GOSSIP_SENDER_MAIN, 5000);
    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, "-= CLOSE =-", GOSSIP_SENDER_MAIN, 7000);

    player->PlayerTalkClass->SendGossipMenu(DEFAULT_GOSSIP_MESSAGE, _Creature->GetObjectGuid()); // 907
    return true;
}
...
void SendDefaultMenu_Teleporter(Player *player, Creature *_Creature, uint32 action)
{
    ...
    case 1006: // Orgrimmar
        player->CLOSE_GOSSIP_MENU();
        if (player->GetTeam() == HORDE)
            player->TeleportTo(1, 1569.696f, -4431.340f, 6.552f, 0.49f);
        break;
    ...
    case 5003: // Dark Portal
        player->CLOSE_GOSSIP_MENU();
        player->TeleportTo(530, -249.756f, 1015.999f, 54.326f, 1.592f);
        break;
...
void AddSC_Teleporter()
{
    ...
    pNewScript->Name = "Teleporter";
    pNewScript->pGossipHello = &GossipHello_Teleporter;
    pNewScript->pGossipSelect = &GossipSelect_Teleporter;
